...
### Быстрый запуск нагрузочного теста

Для удобства создан скрипт `run_load_test.sh`:

```bash
# Базовый запуск (RPS=1000, duration=30s)
./run_load_test.sh

# Кастомные параметры
./run_load_test.sh --rps 500 --duration 60

# Строгий режим с проверкой latency ≤ 1s
./run_load_test.sh --strict
```

Скрипт автоматически проверит доступность сервиса перед запуском теста.
